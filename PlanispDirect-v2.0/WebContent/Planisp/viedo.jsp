...
<div id="eventlist">
 	<c:forEach var="event" items="${events}">
		<div id="${event.s_contentid}" style="background-color:#FFFFFF;" class="_loop">
			<li id="${event.s_starttime}">${event.s_starttime}</li>
			<li id="two">${event.s_name}</li>
			<!-- <li id="three">回看</li> -->
		</div>
	</c:forEach> 
</div>
	<script type="text/javascript">
		...
		for (var index = 0; index  < divs.length-1; index ++) {
			var timelist = divs.item(index).childNodes.item(0).innerHTML;
		...
		function timeListenner(){
			...
			var time = divs.item(c+1).childNodes.item(0).innerHTML;
			var arr = time.split(":");
			var h = parseInt(arr[0]);
			var m = parseInt(arr[1]);
			if(hour>h||hour==h&&min>=m){
				...
				_oldEvent.getElementsByTagName("li").item(0).innerHTML=_oldEvent.getElementsByTagName("li").item(0).id;
